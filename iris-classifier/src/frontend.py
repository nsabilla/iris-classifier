...
st.markdown('Created By: Nurul Najwa Sabilla')
st.divider()

# content
st.subheader('Just type the value, and get the result❤️')
...
with st.form(key='iris_form'):
    ...
    if submit_button:
        ...
        with st.spinner('Predicting...'):
            # do something
            response = requests.post('http://localhost:8000/predict', json=data) # di integrasikan
            result = response.json()
            
        # if success
        if result['status'] == 'OK':
            st.success(result['prediction']) # harus sama kaya backend
            st.balloons()
        else:
            st.error(result['message']) # harus sama kaya backend
